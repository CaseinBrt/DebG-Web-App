...
def process_image(filename):
    ...
    model_input_size = (512, 512)  # Adjust this to match your model's expected input size
...
def index():
    if request.method == 'POST':
        ...
        if file:
            ...
            # Check if the deblurring results meet the threshold
            if psnr < 23:
                return jsonify({
                    'warning': True,
                    'original': filename,
...
def deblur_region():
    ...
    # Check if the PSNR is below the threshold
    if psnr < 23:
        return jsonify({
            'warning': True,
...
if __name__ == '__main__':
    port = int(os.environ.get("PORT", 3505))
